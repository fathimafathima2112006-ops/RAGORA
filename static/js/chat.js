...
async function settingsView(){const u=window.RAGORA_USER||{};return `${layout('Settings','Tune the workspace without hiding the technical controls from your viva.','')}<div class="settings-grid"><div class="panel settings-card rf-account-card"><div class="panel-title"><span>ACCOUNT</span></div><div class="rf-account-row"><img class="rf-account-avatar" src="${u.picture||''}" referrerpolicy="no-referrer" alt=""><div class="rf-account-meta"><strong>${esc(u.name||'Signed-in user')}</strong><span class="rf-account-status">● Signed in with Google</span></div></div><a href="/logout" class="btn secondary rf-signout-btn">Sign out</a></div><div class="panel settings-card"><div class="panel-title"><span>RAG CONFIGURATION</span></div>${[['Chunk Size','900 characters'],['Chunk Overlap','120 characters'],['Top-K','3 chunks'],['Similarity threshold','0.16']].map(x=>`<label><span>${x[0]}</span><b>${x[1]}</b></label>`).join('')}<details><summary>Advanced RAG Settings</summary><p>Hybrid retrieval uses TF-IDF, BM25, keyword overlap, reciprocal-rank fusion and a lexical rerank pass.</p></details></div><div class="panel settings-card"><div class="panel-title"><span>SYSTEM STATUS</span></div>${[['LLM','Groq / GPT OSS','connected'],['Retrieval','Hybrid RRF + rerank','connected'],['Document processor','PDF / DOCX / TXT +','ready'],['Web research','External search fallback','available']].map(x=>`<div class="status-line"><span><b>${x[0]}</b><small>${x[1]}</small></span><em class="status-dot-text">● ${x[2]}</em></div>`).join('')}</div></div>`}
