...
async function renderChat(){el('appView').innerHTML=chatView();await loadDocs().catch(()=>[]);bindChat();bindCardTilt();}
function bindCardTilt(){if(matchMedia('(prefers-reduced-motion: reduce)').matches||matchMedia('(hover: none)').matches)return;document.querySelectorAll('.quick-grid button').forEach(card=>{card.addEventListener('pointermove',e=>{const r=card.getBoundingClientRect(),x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;card.style.setProperty('--tilt-y',`${x*5}deg`);card.style.setProperty('--tilt-x',`${-y*4}deg`)});card.addEventListener('pointerleave',()=>{card.style.setProperty('--tilt-x','0deg');card.style.setProperty('--tilt-y','0deg')})})}\nfunction renderConversationList(){const box=el('conversationList');if(!box)return;box.innerHTML=state.conversations.length?state.conversations.map(c=>`<button class="conv-item ${c.id===state.currentConversationId?'active':''}" data-id="${c.id}"><span>${esc(c.title)}</span><small>${esc(c.created_at||'')}</small></button>`).join(''):empty('No conversations','Start a new grounded chat.','');box.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>openConversation(+b.dataset.id));}
