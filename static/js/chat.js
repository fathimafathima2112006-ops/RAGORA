const state={currentConversationId:null,view:'chat',docs:[],conversations:[],lastRetrieval:null,activeRequest:null,lastUserPrompt:'',lastMode:'auto',voiceMode:false,voiceListening:false};
...
function renderMessage(role,content,usedWeb=false,sources=[],meta={},citations=[]){
  ...
  if(role==='assistant'){
    ...
    metaRow.innerHTML=`
      ...
      <button class="copy-btn">Copy</button><button class="speak-btn" title="Read answer aloud">🔊</button><button class="copy-btn regenerate-btn">Regenerate</button><button class="feedback-btn" data-vote="up" title="Helpful">♡</button><button class="feedback-btn" data-vote="down" title="Not helpful">♧</button>`;
    ...
    speakBtn?.addEventListener('click',()=>speakAnswer(content,speakBtn));
    ...
    if(state.voiceMode) setTimeout(()=>speakAnswer(content,metaRow.querySelector('.speak-btn')),80);
