...
class Config:
    ...
    WEB_MODEL = _env_text("WEB_MODEL", GROQ_MODEL)
    ...
    LLM_TIMEOUT = _env_int("LLM_TIMEOUT", 45, minimum=5, maximum=90)
    MAX_OUTPUT_TOKENS = _env_int("MAX_OUTPUT_TOKENS", 384, minimum=64, maximum=700)
    MAX_HISTORY_MESSAGES = _env_int("MAX_HISTORY_MESSAGES", 6, minimum=1, maximum=8)
    ...
    # Per-file upload ceiling. The UI uploads files sequentially, so each file can be up to 500 MB.
    # Vercel may impose a smaller platform request limit; Render/self-hosted deployments can use this full ceiling.
    MAX_FILE_SIZE_MB = _env_int("MAX_FILE_SIZE_MB", 4 if _env_text("VERCEL") == "1" else 500, minimum=1, maximum=500)
    MAX_CONTENT_LENGTH = MAX_FILE_SIZE_MB * 1024 * 1024
    ALLOWED_EXTENSIONS = {
        "pdf", "docx", "pptx", "txt", "md", "csv", "xlsx", "json",
